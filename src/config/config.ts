...
const defaultApiKey = process.env.AFIP_API_KEY || (() => {
  const generatedKey = `dev-${Date.now()}-${Math.random().toString(36).substring(2, 15)}`;
  console.warn(`⚠️  No se encontró AFIP_API_KEY en variables de entorno. Usando clave temporal: ${generatedKey}`);
  console.warn(`⚠️  En producción, DEBES configurar una API key segura en las variables de entorno.`);
  return generatedKey;
})();
