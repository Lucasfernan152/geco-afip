...
export async function consultarPadron(
  ...
  try {
    ...
    if (!ta || !ta.token || !ta.sign) {
      return {
        success: false,
        error: 'No se pudo obtener ticket de acceso de AFIP. Verifique que el certificado esté configurado correctamente.',
      };
    }
    
    // URL del servicio según ambiente
    const wsdlUrl = config.environment === 'produccion'
    ...
    logger.info('Datos de persona (datosGenerales):', JSON.stringify(persona, null, 2).substring(0, 1000));
    
    // Parsear datos
    logger.info('persona.razonSocial:', persona.razonSocial);
    logger.info('persona.nombre:', persona.nombre);
    logger.info('persona.domicilioFiscal:', persona.domicilioFiscal ? JSON.stringify(persona.domicilioFiscal).substring(0, 200) : 'N/A');
    logger.info('persona.impuesto:', persona.impuesto ? JSON.stringify(persona.impuesto).substring(0, 300) : 'N/A');
    ...
    if (!datos.razonSocial && !datos.domicilio) {
      ...
      return {
        success: false,
        error: 'El CUIT no tiene datos registrados en el padrón de AFIP o tiene pendiente la constitución del domicilio fiscal electrónico.',
      };
    }
    ...
    if (error.message && error.message.includes('No autorizado')) {
      return {
        success: false,
        error: 'No está autorizado para consultar el padrón. Verifique que el servicio ws_sr_padron_a5 esté habilitado en AFIP.',
      };
    }
